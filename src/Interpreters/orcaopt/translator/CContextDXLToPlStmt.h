...
private:
	...
	struct SCTEConsumerInfo
	{
		...
		duckdb_libpgquery::PGList *m_cte_consumer_list;

		// ctor
		SCTEConsumerInfo(duckdb_libpgquery::PGList *plan_cte) : m_cte_consumer_list(plan_cte)
		{
		}
	...
	duckdb_libpgquery::PGList **m_rtable_entries_list;

	// list of oids of partitioned tables
	duckdb_libpgquery::PGList *m_partitioned_tables_list;
	...
	duckdb_libpgquery::PGList **m_subplan_entries_list;
	...
	// into clause
	duckdb_libpgquery::PGIntoClause *m_into_clause;
...
public:
	...
	CContextDXLToPlStmt(CMemoryPool *mp, CIdGenerator *plan_id_counter,
						...
						duckdb_libpgquery::PGList **rtable_entries_list,
						duckdb_libpgquery::PGList **subplan_entries_list);
	...
	duckdb_libpgquery::PGList *GetCTEConsumerList(ULONG cte_id) const;

	// return list of range table entries
	duckdb_libpgquery::PGList *GetRTableEntriesList();
	...
	duckdb_libpgquery::PGList *GetNumPartitionSelectorsList() const;

	duckdb_libpgquery::PGList *GetSubplanEntriesList();
	...
	void AddRTE(duckdb_libpgquery::PGRangeTblEntry *rte, BOOL is_result_relation = false);
	...
	void AddCtasInfo(duckdb_libpgquery::PGIntoClause *into_clause, GpPolicy *distribution_policy);

	// into clause
	duckdb_libpgquery::PGIntoClause *
	GetIntoClause() const
	{

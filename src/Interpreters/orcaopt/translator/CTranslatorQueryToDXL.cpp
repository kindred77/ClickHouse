...
CTranslatorQueryToDXL::CTranslatorQueryToDXL(
	...
	const CMappingVarColId *var_colid_mapping, PGQuery *query, ULONG query_level,
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::CheckUnsupportedNodeTypes(PGQuery *query)
{
	static const SUnsupportedFeature unsupported_features[] = {
		{T_PGRowExpr, GPOS_WSZ_LIT("ROW EXPRESSION")},
		{T_PGRowCompareExpr, GPOS_WSZ_LIT("ROW COMPARE")},
		{T_PGFieldSelect, GPOS_WSZ_LIT("FIELDSELECT")},
		{T_PGFieldStore, GPOS_WSZ_LIT("FIELDSTORE")},
		{T_PGCoerceToDomainValue, GPOS_WSZ_LIT("COERCETODOMAINVALUE")},
	...
	INT non_default_collation = CheckCollation((PGNode *) query);
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::CheckSirvFuncsWithoutFromClause(PGQuery *query)
{
	...
	if (HasSirvFunctions((PGNode *) query->targetList))
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
...
//---------------------------------------------------------------------------
BOOL
CTranslatorQueryToDXL::HasSirvFunctions(PGNode *node) const
{
	GPOS_ASSERT(NULL != node);

	PGList *function_list = ExtractNodesExpression(
		node, T_PGFuncExpr, true /*descendIntoSubqueries*/);
	PGListCell *lc = NULL;
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::CheckSupportedCmdType(PGQuery *query)
{
	...
	if (PG_CMD_SELECT == query->commandType)
	{
		...
		// 	query->parentStmtType == PG_PARENTSTMTTYPE_CTAS)
		// {
		// 	GPOS_RAISE(
		...
		// }
		// if (query->parentStmtType == PG_PARENTSTMTTYPE_COPY)
		// {
		// 	GPOS_RAISE(
		// 		gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
		// 		GPOS_WSZ_LIT(
		...
		// }
		// if (query->parentStmtType == PG_PARENTSTMTTYPE_REFRESH_MATVIEW)
		// {
		// 	GPOS_RAISE(
		// 		gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
	...
	static const SCmdNameElem unsupported_commands[] = {
		{PG_CMD_UTILITY, GPOS_WSZ_LIT("UTILITY command")}};
...
CTranslatorQueryToDXL::CheckRangeTable(PGQuery *query)
{
	...
	foreach(lc, query->rtable)
	{
		...
		// {
		// 	GPOS_ASSERT(PG_RTE_SUBQUERY == rte->rtekind);
...
CTranslatorQueryToDXL::TranslateSelectQueryToDXL()
{
	...
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
				   GPOS_WSZ_LIT("SetOperationStmt not supported yet!"));
...
CDXLNode *
CTranslatorQueryToDXL::TranslateSelectProjectJoinToDXL(
	PGList *target_list, PGFromExpr *from_expr,
	IntToUlongMap *sort_group_attno_to_colid_mapping,
	IntToUlongMap *output_attno_to_colid_mapping, PGList *group_clause)
{
...
// CTranslatorQueryToDXL::TranslateInsertQueryToDXL()
// {
// 	GPOS_ASSERT(PG_CMD_INSERT == m_query->commandType);
...
// CTranslatorQueryToDXL::TranslateCTASToDXL()
// {
// 	GPOS_ASSERT(PG_CMD_SELECT == m_query->commandType);
...
// 		}
// 		PGAttrNumber resno = target_entry->resno;
// 		int var_typmod = ExprTypeMod((PGNode *) target_entry->expr);
...
// 			{
// 				PGAttrNumber attno = m_query->intoPolicy->attrs[ul];
// 				GPOS_ASSERT(0 < attno);
...
//---------------------------------------------------------------------------
// CWStringDynamic *
// CTranslatorQueryToDXL::ExtractStorageOptionStr(PGDefElem *def_elem)
// {
...
// CTranslatorQueryToDXL::TranslateUpdateQueryToDXL()
// {
// 	GPOS_ASSERT(PG_CMD_UPDATE == m_query->commandType);
// 	GPOS_ASSERT(0 < m_query->resultRelation);
...
CDXLWindowSpecArray *
CTranslatorQueryToDXL::TranslateWindowSpecToDXL(
	PGList *window_clause, IntToUlongMap *sort_col_attno_to_colid_mapping,
...
CDXLNode *
CTranslatorQueryToDXL::TranslateWindowToDXL(
	CDXLNode *child_dxlnode, PGList *target_list, PGList *window_clause,
	...
	IntToUlongMap *output_attno_to_colid_mapping)
{
		...
		if (!target_entry->resjunk)
		{
					...
					GPOS_NEW(m_mp) CDXLNode(
						...
						GPOS_NEW(m_mp) CDXLScalarIdent(
							m_mp, GPOS_NEW(m_mp) CDXLColRef(
									  ...
									  colid,
									  GPOS_NEW(m_mp) CMDIdGPDB(ExprType(
										  (PGNode *) target_entry->expr)),
									  ExprTypeMod(
										  (PGNode *) target_entry->expr))));
			...
			else
			{
				...
				omitted_target_entries =
					LAppend(omitted_target_entries, target_entry);
				resno_list = LAppendInt(resno_list, resno);

				project_elem_dxlnode->Release();
			}
		}
		else if (IsA(target_entry->expr, PGWindowFunc))
		{
			// computed columns used in the order by clause
			project_list_dxlnode->AddChild(project_elem_dxlnode);
		}
		else if (!IsA(target_entry->expr, PGVar))
		{
	...
	PGListCell *lc_resno = NULL;
	forboth(lc, omitted_target_entries, lc_resno, resno_list)
	{
		PGTargetEntry *target_entry = (PGTargetEntry *) lfirst(lc);
		INT resno_ = (INT) lfirst_int(lc_resno);

		PGTargetEntry *te_window_spec =
			CTranslatorUtils::GetWindowSpecTargetEntry(
				(PGNode *) target_entry->expr, window_clause, target_list);
		if (NULL != te_window_spec)
		{
			...
			StoreAttnoColIdMapping(output_attno_to_colid_mapping, resno_, colid);
			AddSortingGroupingColumn(target_entry,
									 sort_col_attno_to_colid_mapping, colid);
		}
	}
	...
	{
		GPDBFree(omitted_target_entries);
	}
...
ULongPtrArray *
CTranslatorQueryToDXL::TranslatePartColumns(
	PGList *partition_by_clause, IntToUlongMap *col_attno_colid_mapping) const
{
	...
	foreach(lc, partition_by_clause)
	{
		...
		GPOS_ASSERT(IsA(partition_clause, PGSortGroupClause));
		PGSortGroupClause *sort_group_clause =
...
CDXLNodeArray *
CTranslatorQueryToDXL::TranslateSortColumsToDXL(
	PGList *sort_clause, IntToUlongMap *col_attno_colid_mapping) const
{
	...
	foreach(lc, sort_clause)
	{
		...
		PGSortGroupClause *sort_group_clause =
			(PGSortGroupClause *) node_sort_clause;
	...
	CDXLNode *child_dxlnode, IntToUlongMap *grpcols_to_colid_mapping)
{
	...
	if (NULL != limit_count)
	{
		limit_count_dxlnode->AddChild(TranslateExprToDXL((PGExpr *) limit_count));
	}
	...
	if (NULL != limit_offset_node)
	{
		limit_offset_dxlnode->AddChild(
			TranslateExprToDXL((PGExpr *) limit_offset_node));
	}
...
void
CTranslatorQueryToDXL::AddSortingGroupingColumn(
	PGTargetEntry *target_entry, IntToUlongMap *sort_grpref_to_colid_mapping,
	ULONG colid) const
{
	if (0 < target_entry->ressortgroupref)
	{
...
static BOOL
ExpressionContainsMissingVars(const PGExpr *expr, CBitSet *grpby_cols_bitset)
{
	if (IsA(expr, PGVar) && !grpby_cols_bitset->Get(((PGVar *) expr)->varattno))
	{
		return true;
	}
...
CDXLNode *
CTranslatorQueryToDXL::CreateSimpleGroupBy(
	PGList *target_list, PGList *group_clause, CBitSet *grpby_cols_bitset,
	...
	IntToUlongMap *output_attno_to_colid_mapping)
{
	...
	foreach(lc, target_list)
	{
		...
		PGTargetEntry *te_equivalent =
			CTranslatorUtils::GetGroupingColumnTargetEntry(
				(PGNode *) target_entry->expr, group_clause, target_list);
		...
		else if (IsA(target_entry->expr, PGAggref))
		{
			if (IsA(target_entry->expr, PGAggref) &&
				((PGAggref *) target_entry->expr)->aggdistinct &&
				!IsDuplicateDqaArg(dqa_list, (PGAggref *) target_entry->expr))
			{
				dqa_list = LAppend(dqa_list,
										 CopyObject(target_entry->expr));
				num_dqa++;
			}
			...
			if (has_grouping_sets)
			{
				...
				foreach(ilc, ((PGAggref *) target_entry->expr)->aggdirectargs)
				{
					if (ExpressionContainsMissingVars((PGExpr *) lfirst(ilc),
													  grpby_cols_bitset))
					{
						((PGAggref *) target_entry->expr)->aggdirectargs = NIL;
						break;
					}
				}
			}
		...
		if (is_grouping_col || IsA(target_entry->expr, PGAggref))
		{
...
//---------------------------------------------------------------------------
BOOL
CTranslatorQueryToDXL::IsDuplicateDqaArg(PGList *dqa_list, PGAggref *aggref)
{
	...
	foreach(lc, dqa_list)
	{
		...
		if (Equals(aggref->args, ((PGAggref *) node)->args))
		{
			return true;
		}
	}
...
CDXLNode *
CTranslatorQueryToDXL::TranslateGroupingSets(
	PGFromExpr *from_expr, PGList *target_list, PGList *group_clause, BOOL has_aggs,
	IntToUlongMap *sort_grpref_to_colid_mapping,
	IntToUlongMap *output_attno_to_colid_mapping)
{
...
)
{
	...
	for (ULONG ul = 0; ul < num_of_grouping_sets; ul++)
	{
		...
		PGList *target_list_copy = (PGList *) CopyObject(target_list);
...
CTranslatorQueryToDXL::DXLDummyConstTableGet() const
{
	...
	PGConst *const_expr =
		(PGConst *) MakeBoolConst(true /*value*/, false /*isnull*/);
...
// CDXLNode *
// CTranslatorQueryToDXL::TranslateSetOpToDXL(
// 	PGNode *setop_node, PGList *target_list,
// 	IntToUlongMap *output_attno_to_colid_mapping)
// {
...
CTranslatorQueryToDXL::CreateDXLSetOpFromColumns(
	EdxlSetOpType setop_type, PGList *output_target_list,
...
//---------------------------------------------------------------------------
BOOL
CTranslatorQueryToDXL::SetOpNeedsCast(PGList *target_list,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateSetOpChild(PGNode *child_node,
										   ...
										   PGList *target_list)
{
	...
	if (IsA(child_node, PGRangeTblRef))
	{
	...
	// CHAR *temp_str =
	// 	(CHAR *) NodeToString(const_cast<PGNode *>(child_node));
	const auto& node_str = NodeToString(const_cast<PGNode *>(child_node));
	CHAR *temp_str =
		const_cast<CHAR *>(node_str.c_str());
	CWStringDynamic *str =
...
CTranslatorQueryToDXL::TranslateFromExprToDXL(PGFromExpr *from_expr)
{
	...
	// translate the quals
	PGNode *qual_node = from_expr->quals;
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateFromClauseToDXL(PGNode *node)
{
	GPOS_ASSERT(NULL != node);

	if (IsA(node, PGRangeTblRef))
	{
		...
		static const SRTETranslator dxlop_translator_func_mapping_array[] = {
			{PG_RTE_RELATION, &CTranslatorQueryToDXL::TranslateRTEToDXLLogicalGet},
			{PG_RTE_VALUES, &CTranslatorQueryToDXL::TranslateValueScanRTEToDXL},
			{PG_RTE_CTE, &CTranslatorQueryToDXL::TranslateCTEToDXL},
			{PG_RTE_SUBQUERY, &CTranslatorQueryToDXL::TranslateDerivedTablesToDXL},
			{PG_RTE_FUNCTION, &CTranslatorQueryToDXL::TranslateTVFToDXL},
		};
	...
	if (IsA(node, PGJoinExpr))
	{
		return TranslateJoinExprInFromToDXL((PGJoinExpr *) node);
	}

	//CHAR *sz = (CHAR *) NodeToString(const_cast<PGNode *>(node));
	const auto& node_str = NodeToString(const_cast<PGNode *>(node));
	CHAR *sz = const_cast<CHAR *>(node_str.c_str());
	CWStringDynamic *str =
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::UnsupportedRTEKind(PGRTEKind rtekind) const
{
	GPOS_ASSERT(!(PG_RTE_RELATION == rtekind || PG_RTE_CTE == rtekind ||
				  PG_RTE_FUNCTION == rtekind || PG_RTE_SUBQUERY == rtekind ||
				  PG_RTE_VALUES == rtekind));

	static const SRTENameElem rte_name_map[] = {
		{PG_RTE_JOIN, GPOS_WSZ_LIT("RangeTableEntry of type Join")}/* ,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateRTEToDXLLogicalGet(const PGRangeTblEntry *rte,
												   ULONG rt_index,
...
)
{
	if (false == rte->inh)
	{
		GPOS_ASSERT(PG_RTE_RELATION == rte->rtekind);
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::NoteDistributionPolicyOpclasses(const PGRangeTblEntry *rte)
{
	...
	if (rte->rtekind == PG_RTE_RELATION)
	{
		...
		PGTupleDescPtr desc = rel->rd_att;
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateValueScanRTEToDXL(const PGRangeTblEntry *rte,
												  ULONG rt_index,
												  ULONG current_query_level)
{
	...
	foreach(lc_tuple, tuples_list)
	{
		...
		foreach(lc_column, tuple_list)
		{
			...
			{
				// extract the datum
				PGConst *const_expr = (PGConst *) expr;
				CDXLDatum *datum_dxl =
			...
			else
			{
				...
				if (0 == tuple_pos)
				{
					...
					CDXLColDescr *dxl_col_descr = GPOS_NEW(m_mp) CDXLColDescr(
						m_mp, mdname, colid, col_pos_idx + 1 /* attno */,
						GPOS_NEW(m_mp) CMDIdGPDB(ExprType((PGNode *) expr)),
						ExprTypeMod((PGNode *) expr), false /* is_dropped */
					);
					dxl_col_descr_array->Append(dxl_col_descr);
				}
			}
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateTVFToDXL(const PGRangeTblEntry *rte,
										 ULONG rt_index,
										 ULONG	//current_query_level
)
{
	...
	PGRangeTblFunction *rtfunc = (PGRangeTblFunction *) linitial(rte->functions);
	...
	if (!IsA(rtfunc->funcexpr, PGFuncExpr) && !is_composite_const)
	{
		...
		CDXLNode *project_elem_dxlnode = TranslateExprToDXLProject(
			(PGExpr *) rtfunc->funcexpr, rte->eref->aliasname,
	...
	if (IsA(rtfunc->funcexpr, PGConst))
	{
		CDXLNode *constValue = m_scalar_translator->TranslateScalarToDXL(
			(PGExpr *) (rtfunc->funcexpr), m_var_to_colid_map);
		tvf_dxlnode->AddChild(constValue);
		return tvf_dxlnode;
	}

	GPOS_ASSERT(IsA(rtfunc->funcexpr, PGFuncExpr));

	PGFuncExpr *funcexpr = (PGFuncExpr *) rtfunc->funcexpr;

	// check if arguments contain SIRV functions
	if (NIL != funcexpr->args && HasSirvFunctions((PGNode *) funcexpr->args))
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
				   GPOS_WSZ_LIT("SIRV functions"));
	}
	...
	foreach(lc, funcexpr->args)
	{
		...
		CDXLNode *func_expr_arg_dxlnode =
			m_scalar_translator->TranslateScalarToDXL((PGExpr *) arg_node,
													  m_var_to_colid_map);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateCTEToDXL(const PGRangeTblEntry *rte,
										 ULONG rt_index,
										 ULONG current_query_level)
{
	...
	const PGList *cte_producer_target_list =
	...
	m_var_to_colid_map->LoadCTEColumns(
		...
		const_cast<PGList *>(cte_producer_target_list));
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateDerivedTablesToDXL(const PGRangeTblEntry *rte,
												   ULONG rt_index,
												   ULONG current_query_level)
{
	PGQuery *query_derived_tbl = rte->subquery;
	GPOS_ASSERT(NULL != query_derived_tbl);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateExprToDXL(PGExpr *expr)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateJoinExprInFromToDXL(PGJoinExpr *join_expr)
{
	...
	PGNode *node = join_expr->quals;

	// translate the join condition
	if (NULL != node)
	{
		join_dxlnode->AddChild(TranslateExprToDXL((PGExpr *) node));
	}
	else
	{
	...
	PGListCell *lc_node = NULL;
	PGListCell *lc_col_name = NULL;
	forboth(lc_node, rte->joinaliasvars, lc_col_name, alias->colnames)
	{
		PGNode *join_alias_node = (PGNode *) lfirst(lc_node);
		// rte->joinaliasvars may contain NULL ptrs which indicates dropped columns
		if (!join_alias_node)
			continue;
		GPOS_ASSERT(IsA(join_alias_node, PGVar) ||
					IsA(join_alias_node, PGCoalesceExpr));
		PGValue *value = (PGValue *) lfirst(lc_col_name);
		CHAR *col_name_char_array = strVal(value);

		// create the DXL node holding the target list entry and add it to proj list
		CDXLNode *project_elem_dxlnode = TranslateExprToDXLProject(
			(PGExpr *) join_alias_node, col_name_char_array);
		project_list_dxlnode->AddChild(project_elem_dxlnode);

		if (IsA(join_alias_node, PGCoalesceExpr))
		{
...
CDXLNode *
CTranslatorQueryToDXL::TranslateTargetListToDXLProject(
	PGList *target_list, CDXLNode *child_dxlnode,
	IntToUlongMap *sort_grpref_to_colid_mapping,
	IntToUlongMap *output_attno_to_colid_mapping, PGList *plgrpcl,
	BOOL is_expand_aggref_expr)
{
	...
	foreach(lc, target_list)
	{
		...
		if (!is_groupby || (is_groupby && is_grouping_col))
		{
			...
			BOOL insist_proj =
				(IsA(target_entry->expr, PGVar) &&
				 ((PGVar *) (target_entry->expr))->varlevelsup > 0 &&
				 !is_grouping_col);
			CDXLNode *project_elem_dxlnode = TranslateExprToDXLProject(
				target_entry->expr, target_entry->resname,
			...
			if (!IsA(target_entry->expr, PGVar) || insist_proj)
			{
		...
		else if (is_expand_aggref_expr && IsA(target_entry->expr, PGAggref))
		{
			vars_list = ListConcat(
				vars_list,
				ExtractNodesExpression((PGNode *) target_entry->expr, T_PGVar,
											 false /*descendIntoSubqueries*/));
		}
		else if (!IsA(target_entry->expr, PGAggref))
		{
			omitted_te_list = LAppend(omitted_te_list, target_entry);
		}
	}
	...
	foreach(lc, omitted_te_list)
	{
		...
		PGTargetEntry *te_grouping_col =
			CTranslatorUtils::GetGroupingColumnTargetEntry(
				(PGNode *) target_entry->expr, plgrpcl, target_list);
	...
	foreach(lc_var, vars_list)
	{
		...
		CDXLNode *project_elem_dxlnode =
			TranslateExprToDXLProject((PGExpr *) var, "?col?");
...
) const
{
	...
	foreach(lc, target_list)
	{
		...
		if (IsA(target_entry->expr, PGGroupingFunc))
		{
			colid = m_context->m_colid_counter->next_id();
		...
		else if (!is_grouping_col && !IsA(target_entry->expr, PGAggref))
		{
			OID oid_type = ExprType((PGNode *) target_entry->expr);
...
CDXLNode *
CTranslatorQueryToDXL::CreateDXLProjectGroupingFuncs(
	PGList *target_list, CDXLNode *child_dxlnode, CBitSet *bitset,
	IntToUlongMap *output_attno_to_colid_mapping,
	UlongToUlongMap *grpcol_index_to_colid_mapping,
...
CDXLNodeArray *
CTranslatorQueryToDXL::CreateDXLOutputCols(
	PGList *target_list, IntToUlongMap *attno_to_colid_mapping) const
{
	GPOS_ASSERT(NULL != target_list);
	GPOS_ASSERT(NULL != attno_to_colid_mapping);
	...
	foreach(lc, target_list)
	{
		...
		IMDId *mdid_type = GPOS_NEW(m_mp)
			CMDIdGPDB(ExprType((PGNode *) target_entry->expr));
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateExprToDXLProject(PGExpr *expr,
												 ...
												 BOOL insist_new_colids)
{
	...
	if (IsA(expr, PGVar) && !insist_new_colids)
	{
...
CTranslatorQueryToDXL::CreateDXLConstValueTrue()
{
	PGConst *const_expr =
		(PGConst *) MakeBoolConst(true /*value*/, false /*isnull*/);
	CDXLNode *dxlnode = TranslateExprToDXL((PGExpr *) const_expr);
	GPDBFree(const_expr);
...
CDXLNode *
CTranslatorQueryToDXL::TranslateGroupingFuncToDXL(
	const PGExpr *expr, CBitSet *bitset,
	UlongToUlongMap *grpcol_index_to_colid_mapping) const
{
	GPOS_ASSERT(IsA(expr, PGGroupingFunc));
	...
	ULONG grouping_idx = GetIntFromValue(node);
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::ConstructCTEProducerList(PGList *cte_list,
												ULONG cte_query_level)
{
	...
	foreach(lc, cte_list)
	{
		...
		PGQuery *cte_query = CQueryMutators::NormalizeQuery(
			m_mp, m_md_accessor, (PGQuery *) cte->ctequery, cte_query_level + 1);

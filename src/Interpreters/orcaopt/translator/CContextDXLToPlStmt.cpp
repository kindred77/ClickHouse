...
#include "Interpreters/orcaopt/translator/CContextDXLToPlStmt.h"
...
CContextDXLToPlStmt::CContextDXLToPlStmt(
	...
	DistributionHashOpsKind distribution_hashops, PGList **rtable_entries_list,
	PGList **subplan_entries_list)
	: m_mp(mp),
										...
										ShareInputScan *share_input_scan)
{
	...
	PGList *cte_plan = ListMake1(share_input_scan);
...
//---------------------------------------------------------------------------
void
CContextDXLToPlStmt::AddRTE(PGRangeTblEntry *rte, BOOL is_result_relation)
{
...
{
	PGList *partition_selectors_list = NIL;
	const ULONG len = m_num_partition_selectors_array->Size();
...
void
CContextDXLToPlStmt::AddCtasInfo(PGIntoClause *into_clause,

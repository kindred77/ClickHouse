        ...
        PGSelectStmt * stmt)
{
    ...
    qry->targetList = target_parser_ptr->transformTargetList(pstate, stmt->targetList, EXPR_KIND_SELECT_TARGET);

    /* mark column origins */
    target_parser_ptr->markTargetListOrigins(pstate, qry->targetList);

    /* transform WHERE */
    qual = clause_parser_ptr->transformWhereClause(pstate, stmt->whereClause, EXPR_KIND_WHERE, "WHERE");

    /* initial processing of HAVING clause is much like WHERE clause */
    qry->havingQual = clause_parser_ptr->transformWhereClause(pstate, stmt->havingClause, EXPR_KIND_HAVING, "HAVING");
    ...
    qry->groupClause = clause_parser_ptr->transformGroupClause(
    ...
    else if (linitial(stmt->distinctClause) == NULL)
    {
        ...
        if (!pstate->p_hasAggs && !pstate->p_hasWindowFuncs && qry->groupClause == NIL && qry->targetList != NIL)
        {
			 ...
			 */
            qry->distinctClause = clause_parser_ptr->transformDistinctToGroupBy(pstate, &qry->targetList, &qry->sortClause, &qry->groupClause);
        }
        else
        {
            qry->distinctClause = clause_parser_ptr->transformDistinctClause(pstate, &qry->targetList, qry->sortClause, false);
        }
        qry->hasDistinctOn = false;
    }
    else
    {
        ...
        qry->distinctClause = clause_parser_ptr->transformDistinctOnClause(pstate, stmt->distinctClause, &qry->targetList, qry->sortClause);

...
PGNode *
FuncParser::ParseComplexProjection(PGParseState *pstate, char *funcname, PGNode *first_arg,
					   int location)
{
	TupleDesc	tupdesc;
	int			i;
...
Oid
FuncParser::FuncNameAsType(duckdb_libpgquery::PGList *funcname)
{
	Oid			result;
	Type		typtup;
	 ...
	 */
	typtup = type_parser.LookupTypeNameExtended(NULL, makeTypeNameFromNameList(funcname),
									NULL, false, false);
	if (typtup == NULL)
		return InvalidOid;
...
FuncParser::func_get_detail(PGList *funcname,
				...
				PGList **argdefaults)		/* optional return value */
{
	FuncCandidateList raw_candidates;
	...
	*true_typeids = NULL;
	...
	if (best_candidate == NULL)
	{
		...
		if (nargs == 1 && fargs != NIL && fargnames == NIL)
		{
			...
			if (OidIsValid(targetType))
			{
				...
				else
				{
					...
					switch (cpathtype)
					{
						case CoercionPathType::COERCION_PATH_RELABELTYPE:
							iscoercion = true;
							break;
				...
				if (iscoercion)
				{
					...
					*true_typeids = argtypes;
					return FuncDetailCode::FUNCDETAIL_COERCION;
				}
			}
		}
		...
		if (raw_candidates != NULL)
		{
			...
			else if (ncandidates > 1)
			{
				 ...
				 */
				if (!best_candidate)
					return FuncDetailCode::FUNCDETAIL_MULTIPLE;
			}
		}
	}
	...
	if (best_candidate)
	{
		...
		*vatype = pform->provariadic;
		...
		if (pform->proisagg)
			result = FuncDetailCode::FUNCDETAIL_AGGREGATE;
		else if (pform->proiswindow)
			result = FuncDetailCode::FUNCDETAIL_WINDOWFUNC;
		else
			result = FuncDetailCode::FUNCDETAIL_NORMAL;
				  ...
				  PGFuncCall *fn, int location)
{
	...
	if (fdresult == FuncDetailCode::FUNCDETAIL_COERCION)
	{
		/*
	...
	else if (fdresult == FuncDetailCode::FUNCDETAIL_NORMAL)
	{
		/*
	...
	else if (fdresult == FuncDetailCode::FUNCDETAIL_AGGREGATE)
	{
		/*
	...
	else if (fdresult == FuncDetailCode::FUNCDETAIL_WINDOWFUNC)
	{
		/*
	...
	else
	{
		 ...
		 */
		if (fdresult == FuncDetailCode::FUNCDETAIL_MULTIPLE)
			ereport(ERROR,
					(errcode(PG_ERRCODE_SYNTAX_ERROR),
	...
	if (fdresult == FuncDetailCode::FUNCDETAIL_NORMAL)
	{
	...
	else if (fdresult == FuncDetailCode::FUNCDETAIL_AGGREGATE && !over)
	{
		/* aggregate function */
	...
	else
	{
		...
		wfunc->winagg = (fdresult == FuncDetailCode::FUNCDETAIL_AGGREGATE);
		...
		if (agg_distinct)
		{
			if (fdresult == FuncDetailCode::FUNCDETAIL_WINDOWFUNC)
				ereport(ERROR,
						(errcode(PG_ERRCODE_FEATURE_NOT_SUPPORTED),
					  ...
					  FuncCandidateList candidates)
{
	...
	for (i = 0; i < nargs; i++)
	{
			 ...
			 current_candidate = current_candidate->next)
		{
			...
			coerce_parser.get_type_category_preferred(current_type,
										&current_category,

...
TableScan::TableScan(
    ...
    Poco::Logger * log_)
    : ISourceStep(DataStream{.header = header})
    , analysis_result(std::move(analysis_result_))
    , virt_column_names(std::move(virt_column_names_))
    , is_final(is_final_)
    , input_order_info(input_order_info_)
...
ProcessorPtr TableScan::createSource(
    ...
    bool use_uncompressed_cache)
{
    ...
    for (const auto & part : parts_with_range)
    {
        auto source = read_type == ReadFromMergeTree::ReadType::InReverseOrder
...
Pipe TableScan::read(
    RangesInDataParts parts_with_range, Names required_columns, ReadFromMergeTree::ReadType read_type,
    size_t max_streams, size_t min_marks_for_concurrent_read, bool use_uncompressed_cache)
{
    if (read_type == ReadFromMergeTree::ReadType::Default && max_streams > 1)
    ...
    if (read_type == ReadFromMergeTree::ReadType::Default && pipe.numOutputPorts() > 1)
...
Pipe TableScan::spreadMarkRangesAmongStreams(
    ...
    const Names & column_names)
{
    ...
    return read(std::move(parts_with_ranges), column_names, ReadFromMergeTree::ReadType::Default,
...
Pipe TableScan::spreadMarkRangesAmongStreamsWithOrder(
    RangesInDataParts && parts_with_ranges,
    const Names & column_names,
...
Pipe TableScan::spreadMarkRangesAmongStreamsFinal(
    RangesInDataParts && parts_with_ranges,
    const Names & column_names,
    ActionsDAGPtr & out_projection)
{
...
void TableScan::initializePipeline(QueryPipeline & pipeline, const BuildQueryPipelineSettings &)
{
    //auto result = selectRangesToRead(prepared_parts);
    auto query_id_holder = MergeTreeDataSelectExecutor::checkLimits(data, analysis_result.parts_with_ranges, context);

    if (analysis_result.parts_with_ranges.empty())
    {
    ...
    Names column_names_to_read = std::move(analysis_result.column_names_to_read);
    //const auto & select = query_info.query->as<ASTSelectQuery &>();
    if (!is_final && analysis_result.sampling.use_sampling)
    {
        /// Add columns needed for `sample_by_ast` to `column_names_to_read`.
        /// Skip this if final was used, because such columns were already added from PK.
        std::vector<String> add_columns = analysis_result.sampling.filter_expression->getRequiredColumns().getNames();
    ...
    if (is_final)
    {
        ...
        pipe = spreadMarkRangesAmongStreamsFinal(
            std::move(analysis_result.parts_with_ranges),
    ...
    if (analysis_result.sampling.use_sampling)
    {
        auto sampling_actions = std::make_shared<ExpressionActions>(analysis_result.sampling.filter_expression);
        pipe.addSimpleTransform([&](const Block & header)
        {
            return std::make_shared<FilterTransform>(
                header,
                sampling_actions,
                analysis_result.sampling.filter_function->getColumnName(),
                false);
        });
    }
    ...
    if (sample_factor_column_queried)
    {
        ...
        column.column = column.type->createColumnConst(0, Field(analysis_result.sampling.used_sample_factor));
...
void TableScan::describeActions(FormatSettings & format_settings) const
{
    //auto result = selectRangesToRead(prepared_parts);
    std::string prefix(format_settings.offset, format_settings.indent_char);
    format_settings.out << prefix << "ReadType: " << readTypeToString(analysis_result.read_type) << '\n';

    if (!analysis_result.index_stats.empty())
    {
        format_settings.out << prefix << "Parts: " << analysis_result.index_stats.back().num_parts_after << '\n';
        format_settings.out << prefix << "Granules: " << analysis_result.index_stats.back().num_granules_after << '\n';
    }
}

void TableScan::describeActions(JSONBuilder::JSONMap & map) const
{
    //auto result = selectRangesToRead(prepared_parts);
    map.add("Read Type", readTypeToString(analysis_result.read_type));
    if (!analysis_result.index_stats.empty())
    {
        map.add("Parts", analysis_result.index_stats.back().num_parts_after);
        map.add("Granules", analysis_result.index_stats.back().num_granules_after);
    }
}

void TableScan::describeIndexes(FormatSettings & format_settings) const
{
    //auto result = selectRangesToRead(prepared_parts);
    auto index_stats = std::move(analysis_result.index_stats);

    std::string prefix(format_settings.offset, format_settings.indent_char);
    if (!index_stats.empty())
    {
        /// Do not print anything if no indexes is applied.
        if (index_stats.size() == 1 && index_stats.front().type == ReadFromMergeTree::IndexType::None)
            return;
        ...
        for (size_t i = 0; i < index_stats.size(); ++i)
        {
            ...
            if (stat.type == ReadFromMergeTree::IndexType::None)
                continue;
...
void TableScan::describeIndexes(JSONBuilder::JSONMap & map) const
{
    //auto result = selectRangesToRead(prepared_parts);
    auto index_stats = std::move(analysis_result.index_stats);
